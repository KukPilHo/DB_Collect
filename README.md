...
복잡한 명령어 없이 대화형 메뉴(`main.py`)를 통해 모든 기능을 손쉽게 실행할 수 있습니다.
...
| 메뉴 | 기능 설명 | Input (필요 조건) | Output |
|:---:|---|---|---|
| **1** | 공장 DB 원본 다운로드 | **[필요 없음]** 인터넷 연결만 되면 공공데이터를 자동 다운로드합니다. | `input/` 폴더 내 원본 파일들 |
| **2** | 공장 DB 전체 파이프라인 실행 | **[필요 없음]** 다운로드된 데이터를 바탕으로 DB를 구축하고 600개 타겟의 이메일을 수집합니다. | `output/outbound_db_<날짜>.xlsx` |
| **3** | 내 엑셀 파일 이메일 보강<br>(최적화/단일 추출 병렬 모드) | **[필수]** 사용자가 별도로 준비한 **엑셀 파일의 경로**를 터미널에 입력해야 합니다. (예: `수행기관조회_20260520.xlsx`)<br>*※ 조건: 엑셀 파일 내에 '회사명', '업체명', '상호명' 중 하나의 컬럼이 반드시 있어야 합니다.* | `output/enriched_fast_<원본>_<날짜>.xlsx` |
| **4** | 현재 DB 수집 상태 확인 | **[필요 없음]** | 터미널에 수집 통계 출력 |
